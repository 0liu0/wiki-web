<template>
  <a-layout>
    <a-layout-sider width="200" style="background: #fff">
      ...
      >
        <!-- 遍历得到所有的目录 -->
        <a-sub-menu v-for="(item,index) in level1" :key="item.id" @titleClick.stop="titleClick(item.id)">
          <template #icon>
            <MailOutlined />
          </template>
          <template #title>{{item.name}}</template>
          <a-menu-item v-for="(child,index) in item.children" :key="child.id">{{child.name}}</a-menu-item>
        </a-sub-menu>
      </a-menu>
    </a-layout-sider>
    <a-layout-content
...
export default defineComponent({
  ...
  setup() {
    ...
    const titleClick = (cid: number) => {
      console.log('儿子被点了'+cid);
    }

    return { ebooks, level1,onOpenChange,titleClick };
  },
});
</script>
